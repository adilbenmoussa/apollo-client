define([
  ...
  graphql_tag_1 = tslib_1.__importStar(graphql_tag_1);
  ...
  describe('Cache', function() {
    ...
    function itWithCacheConfig(message, config, callback) {
      var caches = [
        new __1.InMemoryCache(
          tslib_1.__assign({ addTypename: false }, config, {
            resultCaching: true,
          }),
        ),
        new __1.InMemoryCache(
      ...
      caches.forEach(function(cache, i) {
        it(message + (' (' + (i + 1) + '/' + caches.length + ')'), function() {
    ...
    describe('readQuery', function() {
      itWithInitialData(
        ...
        function(proxy) {
          expect(
            apollo_utilities_1.stripSymbols(
              proxy.readQuery({
      ...
      itWithInitialData(
        ...
        function(proxy) {
          ...
          expect(
            apollo_utilities_1.stripSymbols(proxy.readQuery(options)),
          ).toEqual({ a: 1, b: 2 });
    ...
    describe('writeQuery', function() {
      itWithInitialData('will write some data to the store', [{}], function(
        proxy,
      ) {
        proxy.writeQuery({
          data: { a: 1 },
    ...
    describe('performTransaction', function() {
      itWithInitialData('will not broadcast mid-transaction', [{}], function(
        cache,
      ) {
        var numBroadcasts = 0;
