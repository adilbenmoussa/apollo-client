...
function defaultValueFromVariable(node) {
  throw process.env.NODE_ENV === 'production'
    ...
    : new InvariantError('Variable nodes are not supported by valueFromNode');
}
  ...
  return getInclusionDirectives(selection.directives).every(function(_a) {
    ...
    if (ifArgument.value.kind === 'Variable') {
      ...
      process.env.NODE_ENV === 'production'
        ? invariant(evaledValue !== void 0, 1)
        : invariant(
            evaledValue !== void 0,
...
function getInclusionDirectives(directives) {
  return directives
    ? directives.filter(isInclusionDirective).map(function(directive) {
        ...
        process.env.NODE_ENV === 'production'
          ? invariant(directiveArguments && directiveArguments.length === 1, 2)
          : invariant(
        ...
        process.env.NODE_ENV === 'production'
          ? invariant(ifArgument.name && ifArgument.name.value === 'if', 3)
          : invariant(
...
function getFragmentQueryDocument(document, fragmentName) {
  ...
  if (typeof actualFragmentName === 'undefined') {
    process.env.NODE_ENV === 'production'
      ? invariant(fragments.length === 1, 6)
      : invariant(
          fragments.length === 1,
          'Found ' +
            fragments.length +
